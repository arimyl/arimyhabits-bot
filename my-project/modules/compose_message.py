...
def compose_message(
    type: str, text: str, items: List[dict] = None
) -> TemplateSendMessage:
    """TemplateMessageの選択"""
    if type == "confirm":
        tmp = ConfirmTemplate(text, confirm_actions)
    elif type == "button":
        tmp = ButtonsTemplate(text, actions=compose_type_button(items))
    else:
        tmp
...
def compose_type_button(types: List[dict]) -> List[dict]:
    ...
    buttons_actions = []
    for i in types:
        action = {"type": "message", "label": i["name"], "text": str(i["number"])}
